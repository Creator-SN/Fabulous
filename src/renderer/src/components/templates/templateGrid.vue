<template>
    ...
    >
        ...
        >
            <div class="content-preview" @click="itemClick(item)">
                <fv-shimmer
                ...
                <power-editor
                    v-else
                    :model-value="computeContent(item.minContent)"
            ...
            <div class="bottom-info">
                <div class="bottom-item-row">
                    ...
                    <p class="template-title" :title="item.name">{{ item.name }}</p>
                </div>
                <div class="bottom-item-row">
            ...
            <span v-show="!item.default && multiChoosen" class="icon-block icon" key="multi-col">
                <fv-check-box
                    v-model="item.choosen"
                    background="rgba(140, 148, 228, 1)"
                    @click.capture="chooseCurrent($event, item)"
                ></fv-check-box>
            </span>
        </div>
        <right-menu ref="rightMenu" :theme="theme" :rightMenuWidth="rightMenuWidth">
            <slot name="menu"> </slot>
        </right-menu>
    </div>
</template>
    ...
    methods: {
        async loadTemplates() {
            ...
            for (let el of this.modelValue) {
                ...
                template.show = true
                let res = await this.$api.AcademicController.getTemplateContent(
