<template>
    ...
    >
        <template v-slot:trigger>
            <p class="fabulous-emoji-callout-content">{{ modelValue }}</p>
        </template>
...
// 这里calloutBase之所以加key是因为，slot 的触发器内容被组件内部缓存了, 外部modelValue变化了, 但是组件内部的触发器内容没有变化, 所以需要加key来通过key变化让slot 内容更新.
export default {
    components: {
        calloutBase
    },
