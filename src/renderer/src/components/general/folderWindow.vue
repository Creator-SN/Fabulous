<template>
    <float-window-base v-model="thisValue" :title="title" :theme="theme">
        ...
        <template v-slot:control>
            <fv-button theme="dark" background="rgba(140, 148, 228, 1)" @click="confirm">{{
                local('Confirm')
            }}</fv-button>
            <fv-button :theme="theme" style="margin-left: 5px" @click="thisValue = false">{{
                local('Cancel')
            }}</fv-button>
        </template>
    </float-window-base>
...
export default {
    ...
    methods: {
        async refreshTreeList() {
            ...
            this.FLAT = []
            let result = []
            let groups = await this.$api.AcademicController.getRootGroups(this.currentDataPath)
            let partitions = await this.$api.AcademicController.getRootPartitions(
        ...
        async loadChildren(item) {
            ...
            let groupList = []
            let partitionList = []
            await this.$api.AcademicController.getGroups(this.currentDataPath, item.id)
                .then((res) => {
                    if (res.code === 200) {
            ...
            await this.$api.AcademicController.getPartitions(this.currentDataPath, item.id)
                .then((res) => {
                    if (res.code === 200) {
                        let children = res.data
