    ...
    <div class="local-tree-view-container">
        ...
        >
            <div class="navigation-view-mode-left-block">
                ...
                <p class="title">{{ !modelValue ? local('Notebook') : localPathFolderName }}</p>
            </div>
            <div v-show="modelValue" class="navigation-view-mode-right-block">
                <i
        ...
        <fv-TreeView
            ...
            :foreground="'rgba(245, 78, 162, 0.8)'"
            :item-height="35"
        ...
        <div v-if="!isRemote" v-show="!path" class="local-empty-block">
            <fv-button
                theme="dark"
        ...
        >
            <div>
                ...
                <hr v-if="!isRemote" />
                <span v-if="!isRemote" @click="openFile(rightMenuItem)">
                    <img
                        draggable="false"
                        :src="img.folder"
                        alt=""
...
export default {
    ...
    data() {
        return {
            ...
            FLAT: [],
            copyList: [],
            nw: !this.isRemote ? new NotebookWatcher() : new RemoteNotebookWatcher(),
            rightMenuItem: {},
            show: {
                rightMenu: false
            },
    ...
    computed: {
        ...
        localPathFolderName() {
            ...
            let pathList = this.modelValue.split(/[\\/]/)
            return pathList[pathList.length - 1]
        },
        SourceDisabled() {
    ...
    methods: {
        ...
        eventInit() {
            this.nw.on('watch-path-localTree', (e, { event, path, file }) => {
                ...
                if (event === 'addDir' || event === 'add') {
                    ...
                    let fileItem = this.FLAT.find((it) => this.comparePath(it.filePath, path))
                    if (fileItem) {
                        let parentPath = this.findParentPath(fileItem)
                        if (parentPath.isRoot) {
                            ...
                            if (treeIndex > -1) this.treeList.splice(treeIndex, 1)
                            ...
                            if (parentItem) {
                                let index = parentItem.children.findIndex((it) =>
                                    this.comparePath(it.filePath, fileItem.filePath)
                                )
                                let flatIndex = this.FLAT.findIndex((it) =>
                                    this.comparePath(it.filePath, fileItem.filePath)
                                )
                                if (index > -1) parentItem.children.splice(index, 1)
                                if (flatIndex > -1) this.FLAT.splice(flatIndex, 1)
                            }
                        }
                    }
        ...
        async chooseFolder() {
            await this.$local_api.ConfigController.selectLocalDataSourcePath().then((res) => {
                if (res.code === 200) {
                    this.path = res.data
                }
            })
        ...
        hotPushFLAT(item) {
            let index = this.FLAT.findIndex((it) => this.comparePath(it.filePath, item.filePath))
            if (index > -1) {
        ...
        createFile(dir = null) {
            ...
            if (!dir) {
                ...
                let dirItem = this.FLAT.find((it) => this.comparePath(it.filePath, dir))
                if (!dirItem) return
        ...
        nameJudge(target) {
            ...
            if (target.filePath) {
                let matchItem = this.FLAT.find((it) => {
                    ...
                    return filePath === `${dir}/${target.name}` && it.id !== target.id
                })
        ...
        showRename(item) {
            ...
            setTimeout(() => {
                ...
                textbox.focus()
                let input = textbox.$el.querySelector('input')
                let dotIndex = input.modelValue.lastIndexOf('.')
        ...
        newFileConfirm(target) {
            ...
            this.Auto.NotebookController.createDocument(this.uri, url, JSON.stringify(fbn))
                .then((res) => {
                    if (res.code === 200) {
                        this.removeTmp()
                    } else
        ...
        renameConfirm(target) {
            if (target.isDir)
                this.Auto.NotebookController.updateDirectoryInfo(this.uri, target.filePath, {
                    name: target.name
                })
                    .then((res) => {
                        if (res.code !== 200) {
                ...
                this.Auto.NotebookController.updateDocumentInfo(this.uri, target.filePath, {
                    name: target.name
                })
                    .then((res) => {
                        if (res.code !== 200) {
        ...
        deleteConfirm(target) {
            ...
            if (target.isDir) {
                this.Auto.NotebookController.removeDirectory(this.uri, target.filePath)
                    .then((res) => {
                        if (res.code !== 200) {
                            console.error(res)
                ...
                this.Auto.NotebookController.removeDocument(this.uri, target.filePath)
                    .then((res) => {
                        if (res.code !== 200) {
                            console.error(res)
        ...
        paste(target) {
            ...
            for (let item of this.copyList) {
                let itemPath = this.isRemote ? this.computeFileName(item.path) : item.name
                if (item.type === 'copy') {
