<template>
    <transition :name="itemCarrier.itemsX.length > 0 ? 'move-right-to-left' : 'move-left-to-right'">
        ...
        >
            <div v-show="!show.panel" class="carrier-flag" @click="show.panel = true">
                ...
                <template v-slot:container>
                    <div class="panel-container">
                        <fv-collapse
                            ...
                            style="margin: 5px"
                            @contextmenu.capture="rightClick($event, itemX.item)"
                        >
                            ...
                            <template v-slot:title="x">
                                <div class="custom-collapse-title">
                                    <p class="title-content h" :title="x.title">{{ x.title }}</p>
                                </div>
                            </template>
                            ...
                            <template v-slot:extension>
                                <fv-tag
                                    :model-value="itemX.item.labels"
                                    :theme="theme"
                                    class="tag-block"
                                    :size="'xsmall'"
                                    style="max-width: 120px; overflow: auto"
                                    @click.capture="$emit('label-click', itemX.item)"
                                ></fv-tag>
                            </template>
                        </fv-collapse>
                    </div>
    ...
    computed: {
        ...
        currentChoosen() {
            ...
            for (let i = 0; i < this.itemCarrier.itemsX.length; i++) {
                if (this.itemCarrier.itemsX[i].choosen) result.push(this.itemCarrier.itemsX[i])
            }
            return result
        },
        Auto() {
        ...
        async confirm() {
            ...
            for (let i = 0; i < this.currentChoosen.length; i++) {
                ...
                for (let page of item.pages) {
                    await this.Auto(uri)
                        .AcademicController.getItemPageContent(uri, item.id, page.id)
                        .then((res) => {
            ...
            for (let i = 0; i < dataList.length; i++) {
                ...
                let res = await this.Auto(targetUri).AcademicController.createItem(
                    targetUri,
                    pureItem
                )
                ...
                res = await this.Auto(targetUri)
                    .AcademicController.updateItemMetadata(targetUri, newItem.id, _metadata)
                    .catch((res) => {
                        console.warn(res.message)
                    })
                ...
                for (let page of item.pages) {
                    let res = await this.Auto(targetUri)
                        .AcademicController.createItemPage(targetUri, newItem.id, page)
                        .catch((res) => {
                            console.warn(res.message)
                        })
                ...
                this.$emit('update-progress', (50 + ((i + 1) / dataList.length) * 50).toFixed(2))
            }
            ...
            if (this.$route.params.id) {
                ...
                await this.Auto(this.currentDataPath)
                    .AcademicController.addItemsToPartition(this.currentDataPath, partitionid, ids)
                    .then(() => {
                        this.revisePdfImporter({
                            // 触发刷新Items
                            counter: this.counter + 1
