...
export default {
    ...
    methods: {
        ...
        outsideClickEvent(event) {
            ...
            const insideButton = this.$el && this.$el.contains(target);
            const insidePdfContainer = this.parent && this.parent.contains(target);
            if (!insideButton && !insidePdfContainer) this.thisValue = false;
        },
    },
};
</script>
