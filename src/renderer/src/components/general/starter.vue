<template>
    <div class="fabulous-starter-container">
        ...
        <transition name="scale-up-to-up">
            <div v-show="step === 0" class="item-block">
                <fv-img :src="img.logo" style="width: 80px; height: auto"></fv-img>
        ...
        <transition name="scale-up-to-up">
            <div v-show="step === 1" class="item-block">
                <fv-img :src="img.language" style="width: 120px; height: auto"></fv-img>
        ...
        <transition name="scale-up-to-up">
            <div v-show="step === 2" class="item-block">
        ...
        <transition name="scale-up-to-up">
            <div v-show="step === 3" class="item-block">
                <fv-img :src="img.dataSource" style="width: 120px; height: auto"></fv-img>
                <p class="title">{{ local(`New Data Source`) }}</p>
                <p class="info" style="max-width: 600px; margin-bottom: 35px; text-align: center">
                    {{
                        local(
                ...
                >
                    <p
                        ...
                        "
                    >
                        {{ path ? path : local('Choose Data Source Directory ...') }}
                    </p>
                </fv-button>
                <p v-show="name && path" class="info">
                    {{ local(`Will create on`) }} [{{ path }}]
                    {{ local('create with new data source') }} [{{ name }}]
                </p>
                <fv-button
                    theme="dark"
                    background="rgba(0, 130, 180, 1)"
        ...
        <transition name="scale-up-to-up">
            <div v-show="step === 4" class="item-block">
                <fv-img :src="img.link" style="width: 120px; height: auto"></fv-img>
                <p class="title">{{ local(`Choose from Exists`) }}</p>
                <p class="info" style="margin-bottom: 25px">
                    {{
                        local(
                ...
                >
                    <p
                        ...
                        "
                    >
                        {{ path ? path : local('Choose Data Source Path ...') }}
                    </p>
                </fv-button>
                <fv-button
                    theme="dark"
...
import dataSourceImg from '@/assets/nav/research.svg';
...
export default {
    ...
    computed: {
        ...mapState(useDataStore, {
            ...
            theme: (state) => state.configState.theme
        }),
