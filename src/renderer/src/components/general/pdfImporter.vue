<template>
    <transition :name="value ? 'move-right-to-left' : 'move-left-to-right'">
        <div v-show="value" class="fabulous-pdf-importer" :class="[{ dark: theme == 'dark' }]">
            <input
                v-show="false"
                type="file"
            ...
            />
            <div class="line">
                <img draggable="false" :src="img.pdf" class="fab-pdf-importer-img-icon" alt="" />
            ...
            <img draggable="false" :src="img.pdf" class="fab-pdf-importer-img-icon-bg" alt="" />
        </div>
    </transition>
</template>
...
defineExpose({
    inputInspectClick: (...args) => proxy.inputInspectClick(...args)
})
</script>
    ...
    methods: {
        ...
        async getPDFData() {
            ...
            if (this.mode === 'item') {
                ...
                for (let i = 0; i < this.$refs.input.files.length; i++) {
                    ...
                    this.progress = ((i + 1) / this.$refs.input.files.length) * 100
                    ...
                    let res = await this.$api.AcademicController.createItem(
        ...
        async dropFiles() {
            ...
            for (let i = 0; i < this.df.length; i++) {
                ...
                _item.pdf = `${_item.id}`
                _item.metadata = _metadata
                let res = await this.$api.AcademicController.createItem(this.currentDataPath, _item)
                if (res.code !== 200) {
                    this.$barWarning(res.message, {
                        status: 'error'
                    })
        ...
        async copyToPartition(itemid) {
            ...
            if (!id) return
            let res = null
            res = await this.$api.AcademicController.addItemsToPartition(this.currentDataPath, id, [
                itemid
            ])
            if (res.code !== 200) {
                this.$barWarning(res.message, {
        ...
        async copyPdf(objURL, id = null) {
            ...
            let pdfid = id
            await this.$api.AcademicController.updateItemPDF(this.currentDataPath, id, blob)
                .then((res) => {
        ...
        async saveMetadata(_metadata, id = null) {
            if (!id) id = this.item.id
            let res = await this.$api.AcademicController.updateItemMetadata(
                this.currentDataPath,
                id,
                _metadata
            )
        ...
        async getTitleMetadata(file) {
            ...
            let pdfMetadata = await this.extractor.getMetadata(URL.createObjectURL(file))
            if (pdfMetadata.title && pdfMetadata.title != '') title = pdfMetadata.title
            else if (pdfMetadata.Title && pdfMetadata.Title != '') title = pdfMetadata.Title
            ...
            if (pdfMetadata.Author && pdfMetadata.Author.indexOf(' ; ') > -1) {
                ...
                authors_str.forEach((el, idx) => {
                    ...
                    _author.last = el.split(' ').length > 1 ? el.split(' ')[1] : ''
