<template>
    <float-window-base v-model="thisShow" :title="local('Diff Preview')" :theme="theme">
        <template v-slot:header>
            <div class="diff-title-block">
                <p class="panel-title" style="font-size: 15px">{{ local('Diff Preview') }}</p>
                <div class="diff-author-info-block">
                    ...
                    <p class="diff-author-info">
                        {{ authorInfo.email ? authorInfo.email : author }}
                    </p>
                    <time-rounder
        ...
        <template v-slot:content>
            <div class="w-p-block" style="overflow: auto">
                ...
                >
                    <div v-show="item.added || item.removed" class="diff-option-block">
                        <fv-check-box
                            ...
                            >{{ local('Keep') }}
                            {{ item.added ? local('Current') : local('Remote') }}
                            {{ local('Banner') }}</fv-check-box
                        >
                    </div>
                    <fv-img :src="item.value" class="diff-notebook-banner-img"></fv-img>
                </div>
                <div
                ...
                >
                    <div v-show="item.added || item.removed" class="diff-option-block">
                        <fv-check-box
                            ...
                            background="rgba(140, 148, 228, 1)"
                            >{{ local('Keep') }}
                            {{ item.added ? local('Current') : local('Remote') }}</fv-check-box
                        >
                    </div>
                    <div
                    ...
                    >
                        <power-editor
                            :model-value="computedContent(item)"
                            :placeholder="local('No content here ...')"
                            :editable="false"
                            :theme="theme"
                            :editorBackground="
                                item.added
                                    ? diffBackground[theme].added
                                    : item.removed
                                      ? diffBackground[theme].removed
                                      : diffBackground.normal
                            "
            ...
            this.timer.diff = setTimeout(() => {
                if (this.source && this.target) {
                    ...
                    let diffTitleList = Diff.diffWords(
                        ...
                        this.target.title ? this.target.title : ''
                    )
                    ...
                    let sourceContent = Object.assign([], this.source.content.content)
                    let targetContent = Object.assign([], this.target.content.content)
        ...
        commit(save = false) {
            ...
            this.diffContentList.forEach((item) => {
                if (item.choosen) {
                    ...
                    result.content.content = result.content.content.concat(content)
                }
            })
