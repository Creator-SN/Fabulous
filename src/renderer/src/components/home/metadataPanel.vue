            ...
            <div class="metadata-container" :class="{ dark: theme == 'dark' }">
                        ...
                        >
                            ...
                            <div class="label-block">
                                <fv-button
                                    ...
                                    :border-radius="12"
                                    style="width: 80px; height: 25px; flex-shrink: 0"
                                    >{{ item.year }}</fv-button
                                >
                                <fv-button
                                    ...
                                    :border-radius="12"
                                    style="width: auto; height: 25px; margin: 0px 5px"
                            ...
                            <div class="extension-block">
                                ...
                                <fv-button
                                    ...
                                    ><i
                                        class="ms-Icon"
                                        :class="[
                                            `ms-Icon--${item.expand ? 'ChevronUp' : 'ChevronDown'}`
                                        ]"
                                    ></i
                                ></fv-button>
                            </div>
                            <transition :name="'abstract-move'">
                                <div v-show="item.expand" class="abstract-block">
                ...
                <div class="metadata-item">
                    ...
                    <fv-text-field
                        ...
                        :focus-border-color="'rgba(140, 148, 228, 0.8)'"
                        :is-box-shadow="true"
                        style="width: 100%; height: 300px; font-family: 'Times New Roman'"
                    ></fv-text-field>
                </div>
                <div class="metadata-item">
        ...
        metadataInit() {
            ...
            for (let key in this.metadata) {
                if (this.item.metadata[key]) {
                    if (
                        Object.prototype.toString.call(this.item.metadata[key]) ===
                        '[object Object]'
                    )
                        this.metadata[key] = this.item.metadata[key]
                    else this.metadata[key] = this.item.metadata[key].toString()
                } else this.metadata[key] = ''
            }
            if (!this.item.metadata.authors) this.metadata.authors = []
        },
        addAuthor(index = 0) {
            this.metadata.authors.splice(index, 0, JSON.parse(JSON.stringify(author)))
        },
        delAuthor(index) {
            ...
            for (let key in required) {
                if (key == 'author') {
                    if (!this.metadata.authors.length > 0) {
                        this.$barWarning(`'${key}' ${this.local('is required')}`, {
                            status: 'warning'
                        })
                        return
                    }
                    ...
                    if (!this.metadata[required[key]]) {
                        this.$barWarning(`'${required[key]}' ${this.local('is required')}`, {
                            status: 'warning'
                        })
                        return
                    }
