<template>
    <div class="forgot-block">
        ...
        <div class="main-block">
            ...
            <div class="s1">
                <img draggable="false" class="logo-text" src="@/assets/logo.svg" alt="" />
                ...
                <fv-button
                    ...
                    :disabled="!lock.forgot || !user.id || !lock.sendCode"
                    @click="sendCode"
                ...
                <div v-show="step === 1" class="verify-id-block">
                    <i class="block-btn ms-Icon ms-Icon--ChevronLeft20" @click="step = 0"></i>
                ...
                >
                <div class="s2">
                    <p class="to-apply" @click="$emit('switch-block', 'login')">
                        {{ local('Login') }}
                    </p>
                    <p class="to-apply" style="margin: 0px 8px">·</p>
                    <p class="to-apply" @click="$emit('switch-block', 'apply')">
    ...
    methods: {
        ...
        async handlerForgot(code) {
            ...
            this.$api.UserController.updatePassword(this.user.id, {
                ...
                .then((res) => {
                    if (res.code === 200) {
                        this.$barWarning(this.local('Update Password Success.'), {
                            status: 'correct'
                        })
                        this.$Go('/login')
        ...
        async sendCode() {
            ...
            this.$api.UserController.sendForgotVerifiedEmail(this.user.id)
                .then((data) => {
                    if (data.code === 200) {
                        this.$barWarning(this.local(`Email code has been sent.`), {
                            status: 'correct'
                        })
                        this.step = 1
                        this.counter = 60
                        this.timer.sendCode = setInterval(() => {
                            this.counter--
                            if (this.counter <= 0) clearInterval(this.timer.sendCode)
                        }, 1000)
                    } else if (data.code === 40012) {
                        this.$barWarning(data.message || this.local(`Code Exists.`), {
                            status: 'error'
                        })
                        this.step = 1
