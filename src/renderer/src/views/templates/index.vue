<template>
    <div class="fabulous-templates-container" :class="[{ dark: theme === 'dark' }]">
        <div class="s-row">
        ...
        <add-template v-model:show="show.add" @finished="getTemplates"></add-template>
        <rename-template :model-value="currentItem" v-model:show="show.rename"></rename-template>
        <template-preview
            :model-value="currentItem"
            v-model:show="show.templatePreview"
        ></template-preview>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        lock() {
            return this.academicLock.templateRead && this.academicLock.deleteTemplate
        },
        SourceDisabled() {
    ...
    methods: {
        ...
        ...mapActions(useAcademicConfig, {
            getRemoteTemplateInfo: 'getTemplateInfo',
            removeRemoteTemplate: 'deleteTemplate'
        }),
        async getTemplates() {
            let res = await this.getRemoteTemplateInfo(this.currentDataPath)
        ...
        deleteTemplate() {
            ...
            this.$infoBox(this.local(`Are you sure to delete this template?`), {
                ...
                theme: this.theme,
                confirm: async () => {
                    let res = await this.removeRemoteTemplate(
        ...
        deleteTemplates() {
            ...
            this.$infoBox(this.local(`Are you sure to delete these templates?`), {
                status: 'error',
